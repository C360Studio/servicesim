...
func Profile() provider.Profile {
	...
	if err != nil {
		...
		// existed.
		panic("provider/perplexity: contracts sub-FS: " + err.Error())
	}
	...
	return provider.Profile{
		...
		// DefaultAuth is left "" (scenario.AuthRequired): provider/perplexity/request.go's
...
// func(Refusal) []byte Profile.ErrorBody needs; named apart from the
// package's existing exported ErrorBody(surface, status, message), which it
// is built from.
//
