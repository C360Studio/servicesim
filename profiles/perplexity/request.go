...
// element of streamModes and has exactly one caller in this package, so
// giving it its own name would only be an obligation with nothing behind
// it.
...
// profiles/exa introduced for its `anyOf[<type>, null]` fields rather than
