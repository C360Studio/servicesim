...
func renderSonar(x *provider.Exchange, p *perplexityProjection, requestModel string) ([]byte, error) {
	...
	resp := completionResponse{
		...
		Choices: []choice{{
			...
			Message: messageWire{Role: roleAssistant, Content: p.Answer},
			Delta:   messageWire{Role: roleAssistant, Content: ""},
		}},
...
func renderImages(images []perplexityImage) []imageWire {
	if len(images) == 0 {
		return nil
	}
...
func renderSonarUsage(u *perplexityUsage) usageWire {
	if u == nil {
		return usageWire{}
	}
	out := usageWire{
	...
	for _, d := range deltas {
		...
		chunk := chatCompletionChunkResponse{
			...
			Choices: []chatCompletionChunkChoice{{
				Index:        0,
				Delta:        messageWire{Role: roleAssistant, Content: d.Text},
				Message:      messageWire{Role: roleAssistant, Content: aggregate.String()},
				FinishReason: nil,
			}},
