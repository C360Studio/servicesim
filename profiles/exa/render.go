...
// projectionBody is how the shared corpus renders through the Exa API. It is the
// decoded form of a turn's `respond:` body.
//
// It is named projectionBody rather than ExaProjection — the name the package design
...
// `results` of its own — see projectionBody.Contents — only the route's own fault
...
type findSimilarProjection struct {
	...
	// when the scenario asks for it — the same rule projectionBody.Context applies
	// for /search.
	Context *string `yaml:"context,omitempty"`
...
func renderSearch(x *provider.Exchange, p *projectionBody, requestID string) ([]byte, error) {
	limit := requestedNumResults(x)

	results := make([]resultWire, 0, len(p.Results))
...
func renderFindSimilar(x *provider.Exchange, p *projectionBody, requestID string) ([]byte, error) {
	fs := p.FindSimilar
	if fs == nil {
	...
	limit := requestedNumResults(x)
	results := make([]resultWire, 0, len(fs.Results))
...
func renderAnswer(x *provider.Exchange, p *projectionBody, requestID string) ([]byte, error) {
	a := p.Answer
	if a == nil {
		a = &answerProjection{}
	}
	...
	citations := make([]citationWire, 0, len(a.Citations))
...
func renderResult(rp *resultProjection) resultWire {
...
func renderOutput(o *outputProjection) *structuredOutputWire {
	out := &structuredOutputWire{Content: o.Content}
	if len(o.Grounding) == 0 {
		return out
	}
	out.Grounding = make([]groundingWire, 0, len(o.Grounding))
	for _, g := range o.Grounding {
		grounding := groundingWire{Field: g.Field, Confidence: g.Confidence}
		if len(g.Citations) > 0 {
			grounding.Citations = make([]groundingCitationWire, 0, len(g.Citations))
			for _, ref := range g.Citations {
				src := scenario.Render(ref)
				grounding.Citations = append(grounding.Citations,
					groundingCitationWire{Title: src.Title, URL: src.URL, ID: src.URL})
			}
		}
...
//
// It is a zero-sized value with no state, so Profile() constructs it directly
// as one of the four in-tree profiles that need no exported name for it.
...
func validateProjection(p *projectionBody, path string) []scenario.Finding {
	var findings []scenario.Finding
