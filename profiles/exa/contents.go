...
func buildContentsIndex(p *projectionBody) contentsIndex {
...
func resolveContentsIdentifiers(s *scenario.Scenario, p *projectionBody, identifiers []string) []contentsResolution {
...
func renderContentsBody(resolutions []contentsResolution, cp *contentsProjection, requestID string) ([]byte, error) {
	results := make([]resultWire, 0, len(resolutions))
	statuses := make([]contentsStatusWire, 0, len(resolutions))
	for _, r := range resolutions {
		if r.result != nil {
			results = append(results, *r.result)
		}
		status := contentsStatusWire{ID: r.id, Status: r.status, Source: r.source}
		if r.status != contentsStatusSuccess {
			status.Error = &contentsStatusErrorWire{Tag: r.errTag, HTTPStatusCode: r.errCode}
		}
