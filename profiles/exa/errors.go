...
// errorResponseWire would be wrong for exactly this status, which is the one a
...
func errorBody(requestID, message, tag string, status int) []byte {
	...
	body, _ := provider.Render(errorResponseWire{RequestID: requestID, Error: message, Tag: tag}, nil, nil)
	return body
}
