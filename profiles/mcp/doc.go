...
// collision documented on profiles/tavily's rawResultProjection. Resolution
// is instead a dedicated pass (resolveContentSources) that validator and
// selectProjection both call.
//
// # Exported surface
//
// Phase 10 unit 5 trimmed this package's exported surface from 19 top-level
// declarations (go doc -short, before the move to profiles/mcp) to 6: what a
// consumer of this profile legitimately names — its Name, its route pattern
// and fault key, Profile(), Routes(), and the finding codes a test asserts
// on directly (CodeParseFailed's block; house rule 5 already said "every one
// is exported so a consumer's test can assert on it directly rather than on
// a status code the simulator could have produced for another reason").
// Everything else — the JSON-RPC protocol error codes (the specification's
// numbers, not this package's invention), the server identity and caching
// defaults, the dispatch method strings, routeMCP's helper, validator, and
// every projection/wire-decode type (contentBlock, projection,
// resourceBlock, resultProjection, toolAnnotations, toolProjection) — is the
// package's own business: nothing outside profiles/mcp names them.
package mcp
