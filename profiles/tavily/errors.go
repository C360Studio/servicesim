...
func errorBody(message string) []byte {
	body, err := provider.Render(errorResponseWire{Detail: errorDetailWire{Error: message}}, nil, nil)
	if err != nil {
		// errorResponseWire is two strings; marshalling it cannot fail. Returning a
