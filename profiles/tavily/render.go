...
// projectionBody is how the shared corpus renders through the Tavily API. It is the
...
func renderSearch(p *projectionBody, req *searchRequest, keys renderKeys) ([]byte, error) {
	body := searchBody{
		Query:        req.Query,
...
// to give.
func renderAnswer(p *projectionBody, req *searchRequest) *string {
...
func renderImages(images []imageProjection, req *searchRequest) []imageWire {
	out := make([]imageWire, 0, len(images))
	if !req.IncludeImages {
		return out
	}
	for _, img := range images {
		entry := imageWire{URL: img.URL}
...
func renderResults(p *projectionBody, req *searchRequest, keys renderKeys) ([]json.RawMessage, error) {
...
// extra_fields added.
func renderResultBody(result resultWire, projected *resultProjection) (json.RawMessage, error) {
...
func renderResponseTime(p *projectionBody, keys renderKeys) float64 {
	if p.ResponseTime != 0 {
...
func renderRequestID(p *projectionBody, keys renderKeys) string {
	if p.RequestID != "" {
		return p.RequestID
	}
...
func renderUsage(p *projectionBody, req *searchRequest) *usageWire {
	if !req.IncludeUsage {
		return nil
	}
	if p.Usage != nil {
		return &usageWire{Credits: p.Usage.Credits}
	}
	credits, ok := creditsPerDepth[req.SearchDepth]
	if !ok {
		credits = 1
	}
	return &usageWire{Credits: credits}
}
...
func renderAutoParameters(p *projectionBody, req *searchRequest) map[string]any {
