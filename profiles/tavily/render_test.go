...
// decoding searchResponseWire would be decoding a shape the renderer never emits.
func TestSearchBodyMatchesSearchResponse(t *testing.T) {
	t.Parallel()

	exported := reflect.TypeOf(searchResponseWire{})
...
`
	...
	var decoded searchResponseWire
...
func TestUnresolvedReferenceDoesNotPanic(t *testing.T) {
	...
	projection := &projectionBody{Results: []resultProjection{{SourceRef: scenario.SourceRef{Ref: "source-z"}}}}
